...
typedef struct ngx_http_myfilter_conf_t {
	ngx_flag_t enable;
} ngx_http_myfilter_conf_t;

typedef struct {
	ngx_int_t add_prefix;  // add_prefix=0表示不需要再返回的包体前加前缀
	                       // add_prefix=1表示应当在包体前添加前缀
	                       // add_prefix=2表示已经添加过前缀
...
static ngx_int_t ngx_http_myfilter_header_filter(ngx_http_request_t *r) {
	...
	// 如果不是返回成功,这时是不需要理会是否加前缀的,直接交由下一个过滤模块处理响应码非200的情况
	...
	if (ctx) {
		// 该请求的上下文已经存在,这说明ngx_http_myfilter_header_filter已经被调用过1次,直接交由下一个过滤模块
		return ngx_http_next_header_filter(r);
	}

	// 获得存储配置项的ngx_http_myfilter_conf_t结构体
	...
	// myfilter过滤模块只能处理Content-type是"text/html"类型的HTTP响应
	//if (r->headers_out.content_type.len >= sizeof("text/plain") - 1 && ngx_strncasecmp(r->headers_out.content_type.data, (u_char *)"text/plain", sizeof("text/plain"))) {
	if (r->headers_out.content_type.len >= sizeof("text/html") - 1 && 
		ngx_strncasecmp(r->headers_out.content_type.data, (u_char *)"text/html", sizeof("text/html")) == 0) {
		// 设置为1表示需要在HTTP包体前加入前缀
		ctx->add_prefix = 1;
		// 当处理模块已经在Content-length中写入了HTTP包体的长度时,由于我们加入了前缀字符串,所以需要把这个字符串的长度也加入
		// 到Content-length中
